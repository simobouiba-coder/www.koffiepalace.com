...
async function initDB() {
  ...
  try {
    ...
    for (const tbl of ['products', 'orders']) {
      await client.query(`
        DROP TRIGGER IF EXISTS set_${tbl}_updated_at ON ${tbl};
...
app.post('/api/create-payment', async (req, res) => {
  try {
    ...
    res.json({ success: true, paymentUrl: payment._links.checkout.href, orderNumber });
...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    console.log(`✅ Contactmail van ${naam} <${email}>`);
    res.json({ success: true });
  } catch (err) {
